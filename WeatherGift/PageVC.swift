...
class PageVC: UIPageViewController {
    ...
    func configureListButton() {
         let safeHeight = view.frame.height - view.safeAreaInsets.bottom
        listButton = UIButton(frame: CGRect(x: (view.frame.width - barButtonWidth) / 2, y: safeHeight - barButtonHeight, width: barButtonWidth, height: barButtonHeight))
    ...
    @IBAction func unwinFromListVC(sender: UIStoryboardSegue) {
        pageControl.numberOfPages = locationsArray.count
        pageControl.currentPage = currentPage
    ...
    //    MARK:- Create VC for UIPageViewController
...
extension PageVC: UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        ...
        if let currentViewController = viewController as? DetailVC { //maybe it needs to be PageVC?
